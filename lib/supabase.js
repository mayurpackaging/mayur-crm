...
// Only fetch last 20 orders on initial load - fast!
export const sbGetOrders   = ()      => sbFetch("crm_orders?order=order_date.desc,created_at.desc&limit=20");
// Load all orders when user opens Orders tab
export const sbGetAllOrders= ()      => sbFetch("crm_orders?order=order_date.desc,created_at.desc");
export const sbGetOrderItems = (oid) => sbFetch(`crm_order_items?order_id=eq.${oid}`);
export const sbGetTargets   = ()     => sbFetch("crm_targets?order=year.desc,month.desc");
